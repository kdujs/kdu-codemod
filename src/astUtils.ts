...
export function getKduOptions(context: Context): Collection<KduOptionsType> {
  const paths: ASTPath<KduOptionsType>[] = []
  const { j, root, filename } = context

  function wrapOptionsInPaths<T>(
    nodes: KduOptionsType
  ): ASTPath<KduOptionsType>[] {
  ...
  for (const p of paths) {
    ...
    for (const subComp of subComponentDefinitions) {
      paths.push(...wrapOptionsInPaths(subComp as KduOptionsType))
    }
  }
